  ...
  return (
    <div className="bg-light me-3 shadow rounded">
      ...
      <h5 className="text-start ms-3">
        <b>Added Artist:</b> {cart.length}
      </h5>
      <h5 className="text-start ms-3 mb-4">
        <b>Total Hire Amount:</b> {total} tk
      </h5>
      {SingerName.map((names) => (
        <ShowName key={names.id} names={names}></ShowName>
      ))}
